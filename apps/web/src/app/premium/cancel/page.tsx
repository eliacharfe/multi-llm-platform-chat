...
const BENEFITS = [
    { title: "All premium AI models", desc: "GPT-5, Claude Opus, Gemini 2.5 Pro and more" },
    { title: "Unlimited messages", desc: "No daily caps, ever" },
    { title: "8,192 token context", desc: "4× more than the free plan" },
    { title: "Deep Search", desc: "Real-time web results injected into any chat" },
    { title: "File & image uploads", desc: "Attach PDFs, code, and images" },
    { title: "Priority response speed", desc: "Faster during peak hours" },
];

function CheckIcon() {
    return (
        <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 text-teal-400 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function XIcon() {
    return (
        <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 text-white/20 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function CancelSubscriptionPage() {
    ...
    if (done) {
        return (
            <main className="min-h-screen bg-[#1a1a1a] text-gray-100 flex items-center justify-center px-5 py-16">
                <div className="w-full max-w-md text-center">
                    ...
                    <h1 className="text-xl font-semibold text-white mb-2">Cancellation scheduled</h1>
                    <p className="text-sm text-white/50 leading-relaxed mb-8">
                        Your Premium access stays active until the end of your current billing period. You won't be charged again.
                    </p>
                    <div className="rounded-2xl border border-white/10 bg-white/5 divide-y divide-white/10 overflow-hidden mb-8 text-left">
                        {[
                            { step: "Now", text: "Premium features remain fully active" },
                            { step: "Billing ends", text: "Access reverts to the free plan automatically" },
                            { step: "Anytime", text: "You can resubscribe from your account page" },
                        ].map(({ step, text }) => (
                            <div key={step} className="flex items-start gap-4 px-4 py-3">
                                <span className="text-xs font-medium text-teal-400 w-20 shrink-0 mt-0.5">{step}</span>
                    ...
                    <button
                        onClick={() => router.push("/")}
                        className="w-full rounded-full border border-white/10 bg-white/5 hover:bg-white/10 transition px-5 py-3 text-sm font-medium text-white/70"
                    >
                        Back to chat
                    </button>
                    <button
                        onClick={() => router.push("/account")}
                        className="mt-2 text-xs text-white/30 hover:text-white/55 transition underline underline-offset-2"
                    >
                        Reactivate from account page
                    </button>
                </div>
            </main>
        );
    ...
    // ── Main cancel page ───────────────────────────────────────────────────
    return (
        <main className="min-h-screen bg-[#1a1a1a] text-gray-100 px-5 py-12">
            <div className="mx-auto max-w-4xl">

                {/* Back */}
                <button
                    onClick={() => router.back()}
                    className="flex items-center gap-2 text-sm text-white/40 hover:text-white/70 transition mb-10"
                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
                    ...
                    <div>
                        <p className="text-xs font-medium text-teal-400 mb-1.5">Premium membership</p>
                        <h1 className="text-2xl font-bold text-white mb-2">Before you go</h1>
                        ...
                        {/* What you'll lose */}
                        <p className="text-xs font-medium text-white/35 mb-3">What you'll lose</p>
                        <div className="rounded-2xl border border-white/10 bg-white/5 divide-y divide-white/10 overflow-hidden mb-5">
                            {BENEFITS.map((b) => (
                                <div key={b.title} className="flex items-start gap-3 px-4 py-3.5">
                                    ...
                                    <span className="ml-auto shrink-0 text-[10px] font-semibold text-teal-400 bg-teal-400/10 border border-teal-400/15 rounded-full px-2 py-0.5 self-center">
                                        Active
                                    </span>
                                </div>
                            ))}
                        ...
                        {/* After cancellation */}
                        <p className="text-xs font-medium text-white/35 mb-3">After cancellation — free plan</p>
                        <div className="rounded-2xl border border-white/10 bg-white/5 divide-y divide-white/10 overflow-hidden">
                            {[
                                ...
                                <div key={line} className="flex items-center gap-3 px-4 py-3">
                                    <XIcon />
                                    <span className="text-sm text-white/40">{line}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:sticky lg:top-8">
                        <div className="rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
                            ...
                            <div className="px-5 py-5">
                                ...
                                {/* Current plan info */}
                                {info && (
                                    <div className="rounded-xl border border-white/10 bg-white/5 divide-y divide-white/10 overflow-hidden mb-5">
                                        <div className="flex items-center justify-between px-4 py-2.5">
                                            <span className="text-xs text-white/45">Current plan</span>
                                            <span className="text-xs font-semibold text-teal-300">⚡ Premium</span>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-2.5">
                                            <span className="text-xs text-white/45">Messages today</span>
                                            <span className="text-xs text-white/65">{info.message_count_today} sent</span>
                                        </div>
                                    </div>
                                )}

                                {error && (
                                    <div className="mb-4 rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-2.5 text-xs text-red-400 leading-relaxed">
                                        {error}
                                    </div>
                                )}
                                ...
                                <div className="flex flex-col gap-2">
                                    <button
                                        ...
                                        onClick={() => router.back()}
                                        disabled={loading}
                                        className="w-full rounded-full bg-linear-to-r from-teal-300 to-cyan-400 px-4 py-2.5 text-sm font-semibold text-black transition hover:opacity-90 disabled:opacity-50"
                                    >
                                        Keep Premium
                                    </button>
                                    <button
                                        ...
                                        disabled={loading}
                                        className="w-full rounded-full border border-white/10 px-4 py-2.5 text-sm font-medium text-white/40 hover:text-white/65 hover:border-white/20 transition disabled:opacity-40"
                                    >
                                        {loading ? (
                                ...
                                <p className="text-center text-xs text-white/25 mt-4">
                                    Need help?{" "}
                                    <a href="mailto:support@multillm.net" className="text-white/40 hover:text-white/60 underline underline-offset-2 transition">
                                        Contact support
                                    </a>
                                </p>
                            </div>
